...
%load('dengq_embryo.mat')
[A,B] = xlsread('dengq_embryo_notch.xlsx');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PULL OUT NOTCH GENES - NOT NEEDED IF LOADING .MAT FILE

%READ FILE NOTCHLIST and STORE INTO CELL ARRAY
fid = fopen('notchlist_dengq.txt');
allData = textscan(fid,'%s','Delimiter','\n');
allData = allData(1);
notch_list = allData{1}';

%B IS SHIFTED FROM A BY 2 DOWN.
dat = zeros(length(notch_list),317);
for typ = 1:length(notch_list)
    type_string = regexp(B,notch_list{typ});
    temp_store = find(~cellfun(@isempty,type_string));
    dat(typ,:) = A(temp_store(1),1:end);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% NORMALIZATION
dat(isnan(dat))=0;  % Convert all the NaNs to zeros
dat = zscore(dat); %Uncomment If you want to use zscores instead..
%dat = normc(dat); %Comment out if you want to use zscores
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
colormap('copper')
cb = colorbar
title(cb,'Z-Scores','FontSize', 15)
xlabel('Cells sorted according to the cluster assigned','FontSize', 15)
title('Heat Map - Deng Q -Embryo - KMedoids Clustering','FontSize', 20)
ax = gca 

ax.YTick = 1:14
ax.YTickLabel = notch_list
ax.XTick = [];
ax.FontSize = 15
...
scatter3(score(:,1), score(:,2),score(:,3),50, idx,'filled')
rotate3d on
colormap(lines(k))
...
if 0
%%%%% TAKE 3
score = tsne(dat, idx, 2, 14,50);
%score = tsne(score, idx, 2, 2,50);
%score = tsne(score, idx, 2, 2,50);
...
legends = {'k=1','k=2','k=3','k
     =4','k=5','k=6','k=7','k=8','k=9','k=10','Optimal Eval Criteria'};
